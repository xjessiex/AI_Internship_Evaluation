...
```{r Calculate genre frequency by creating corpus}
...
for (i in 1:5){
  ...
  # Filter the summary based on the genre
    ...
    # Prevent column name from being saved into the text file
    # names(topfive_dataset) <- NULL
    
    # Compile text file for cleanNLP
    # file_path = here::here("corpus_topfive", paste0(selected_genre, ".txt"))
    # write.table(topfive_dataset, file = file_path, sep =",")
    # 
...
```{r Use cleanNLP package to run annotator}
...
# Examine per-document-per-topic probabilities
  ...
  plot_topwords <- chapters_topics %>%
    ...
    geom_col(show.legend = FALSE) +
    labs(x = "Per-topic-per-word Probabilities", y = "Terms", caption = "Figure 4.1 The terms that are most common within each topic") +
  ...
  plot_topicgen <- chapters_gamma %>%
    mutate(title = reorder(document, gamma * topic)) %>%
    ggplot(aes(factor(topic), gamma, fill = document)) +
    labs(x = "Gamma Probabilities for Genre", y = "Five Topics", caption = "Figure 4.2 The gamma probabilities for each genre") +
    geom_bar(position="dodge",stat="identity") +
    facet_wrap(~ title)

    saveRDS(plot_topicgen, here::here("image", "gamma_LDA.rds"))
